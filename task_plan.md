...
### T5.1: 单股票回测跑通 (1天)
- [ ] 集成所有模块：Tick 合成器 → gRPC → 撮合引擎 → Agent → 绩效
- [ ] 实现 CLI 入口：`python -m agent.main --security-id 600519 --start-date 2026-05-01 --end-date 2026-05-30`
- [ ] 端到端测试：贵州茅台 1 天回测
- [ ] **验收标准**: CLI 命令执行成功，输出完整绩效报告

### T5.2: 多股票回测验证 (1天)
- [ ] 验证不同股票（大盘股、小盘股、创业板）的回测
- [ ] 验证不同时间段（牛市、熊市、震荡）的回测
- [ ] 性能基准：单股票 1 天回测 < 5 分钟
- [ ] **验收标准**: 至少 3 只不同股票回测成功，性能达标
